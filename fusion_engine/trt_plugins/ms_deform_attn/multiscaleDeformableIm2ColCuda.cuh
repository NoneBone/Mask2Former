...
extern "C" void pose_msda_set_enable_optimized_kernel(int32_t enable);
extern "C" int32_t pose_msda_get_enable_optimized_kernel();
...
__device__ __forceinline__ float4 load_float4_custom(float const* ptr)
{
    return *reinterpret_cast<float4 const*>(ptr);
}

__device__ __forceinline__ float4 ms_deform_attn_im2col_bilinear_float4_custom(float const* bottomData,
    int32_t const height, int32_t const width, int32_t const nHeads, int32_t const channels, float const h,
    float const w, int32_t const m, int32_t const cBase)
{
    int32_t const hLow = floorf(h);
    int32_t const wLow = floorf(w);
    int32_t const hHigh = hLow + 1;
    int32_t const wHigh = wLow + 1;

    float const lh = h - hLow;
    float const lw = w - wLow;
    float const hh = 1.0F - lh;
    float const hw = 1.0F - lw;
    ...
    int32_t const wLowPtrOffset = wLow * wStride;
    int32_t const wHighPtrOffset = wLowPtrOffset + wStride;
    int32_t const basePtr = m * channels + cBase;

    float4 v1 = make_float4(0.0F, 0.0F, 0.0F, 0.0F);
    if (hLow >= 0 && wLow >= 0)
    {
        v1 = load_float4_custom(bottomData + hLowPtrOffset + wLowPtrOffset + basePtr);
    }
    float4 v2 = make_float4(0.0F, 0.0F, 0.0F, 0.0F);
    if (hLow >= 0 && wHigh <= width - 1)
    {
        v2 = load_float4_custom(bottomData + hLowPtrOffset + wHighPtrOffset + basePtr);
    }
    float4 v3 = make_float4(0.0F, 0.0F, 0.0F, 0.0F);
    if (hHigh <= height - 1 && wLow >= 0)
    {
        v3 = load_float4_custom(bottomData + hHighPtrOffset + wLowPtrOffset + basePtr);
    }
    float4 v4 = make_float4(0.0F, 0.0F, 0.0F, 0.0F);
    if (hHigh <= height - 1 && wHigh <= width - 1)
    {
        v4 = load_float4_custom(bottomData + hHighPtrOffset + wHighPtrOffset + basePtr);
    }

    float const w1 = hh * hw;
    float const w2 = hh * lw;
    float const w3 = lh * hw;
    float const w4 = lh * lw;
...
__global__ void ms_deformable_im2col_gpu_kernel_v2_split_reduce_float32_custom(float const* __restrict__ dataValue,
    int32_t const* __restrict__ dataSpatialShapes, int32_t const* __restrict__ dataLevelStartIndex,
    float const* __restrict__ dataSamplingLoc, float const* __restrict__ dataAttnWeight, int32_t const spatialSize,
    int32_t const numHeads, int32_t const numQuery, float* __restrict__ dataCol)
{
    constexpr int32_t kVecChannels = 8;
    constexpr int32_t kSamples = 16;

    int32_t const threadId = threadIdx.x;
    int32_t const cVec = threadId % kVecChannels;
    int32_t const sampleIndex = threadId / kVecChannels;
    int32_t const level = sampleIndex / 4;

    int32_t blockIndex = blockIdx.x;
    int32_t const mCol = blockIndex % numHeads;
    blockIndex /= numHeads;
    int32_t const qCol = blockIndex % numQuery;
    int32_t const bCol = blockIndex / numQuery;

    int32_t const samplingIndex = (bCol * numQuery + qCol) * numHeads + mCol;
    int32_t const dataWeightPtr = samplingIndex * kSamples + sampleIndex;
    int32_t const dataLocWPtr = dataWeightPtr << 1;
    int32_t const qidStride = numHeads * 32;
    int32_t const levelStartId = dataLevelStartIndex[level];
    int32_t const spatialHPtr = level << 1;
    int32_t const spatialH = dataSpatialShapes[spatialHPtr];
    int32_t const spatialW = dataSpatialShapes[spatialHPtr + 1];
    int32_t const cBase = cVec << 2;

    float const locW = dataSamplingLoc[dataLocWPtr];
    float const locH = dataSamplingLoc[dataLocWPtr + 1];
    float const weight = dataAttnWeight[dataWeightPtr];
    float const hIm = locH * spatialH - 0.5F;
    float const wIm = locW * spatialW - 0.5F;

    float4 partial = make_float4(0.0F, 0.0F, 0.0F, 0.0F);
    if (hIm > -1.0F && wIm > -1.0F && hIm < spatialH && wIm < spatialW)
    {
        float const* dataValuePtr = dataValue + (bCol * spatialSize + levelStartId) * qidStride;
        float4 const value = ms_deform_attn_im2col_bilinear_float4_custom(
            dataValuePtr, spatialH, spatialW, numHeads, 32, hIm, wIm, mCol, cBase);
        partial.x = value.x * weight;
        partial.y = value.y * weight;
        partial.z = value.z * weight;
        partial.w = value.w * weight;
    }

    int32_t const laneId = threadId & 31;
    int32_t const warpId = threadId >> 5;
    int32_t const channelLane = laneId & (kVecChannels - 1);
    __shared__ float4 warpPartials[4][kVecChannels];
    float4 warpSum = partial;
#pragma unroll
    for (int32_t laneOffset = kVecChannels; laneOffset < 32; laneOffset += kVecChannels)
    {
        float const valueX = __shfl_sync(0xffffffff, partial.x, channelLane + laneOffset);
        float const valueY = __shfl_sync(0xffffffff, partial.y, channelLane + laneOffset);
        float const valueZ = __shfl_sync(0xffffffff, partial.z, channelLane + laneOffset);
        float const valueW = __shfl_sync(0xffffffff, partial.w, channelLane + laneOffset);
        if (laneId < kVecChannels)
        {
            warpSum.x += valueX;
            warpSum.y += valueY;
            warpSum.z += valueZ;
    ...
    if (laneId < kVecChannels)
    {
        warpPartials[warpId][laneId] = warpSum;
    }
    __syncthreads();

    if (threadId < kVecChannels)
    {
        float4 sum = warpPartials[0][threadId];
#pragma unroll
        for (int32_t warp = 1; warp < 4; ++warp)
        {
            float4 const value = warpPartials[warp][threadId];
            sum.x += value.x;
            sum.y += value.y;
            sum.z += value.z;
            sum.w += value.w;
        }
        reinterpret_cast<float4*>(dataCol)[((bCol * numQuery + qCol) * numHeads + mCol) * kVecChannels + threadId]
            = sum;
    }
}

template <typename scalar_t>
__global__ void ms_deformable_im2col_gpu_kernel_v2_custom(int32_t const n, scalar_t const* dataValue,
    int32_t const* dataSpatialShapes, int32_t const* dataLevelStartIndex, scalar_t const* dataSamplingLoc,
    scalar_t const* dataAttnWeight, int32_t const batchSize, int32_t const spatialSize, int32_t const numHeads,
    int32_t const channels, int32_t const numLevels, int32_t const numQuery, int32_t const numPoint, scalar_t* dataCol)
{
    if constexpr (std::is_same<scalar_t, float>::value)
    {
        if ((channels & 3) == 0)
        {
            int32_t const vecChannels = channels >> 2;
            CUDA_KERNEL_LOOP(index, n)
            {
                int32_t _temp = index;
                int32_t const cVec = _temp % vecChannels;
                _temp /= vecChannels;
                int32_t const samplingIndex = _temp;
                int32_t const mCol = _temp % numHeads;
                _temp /= numHeads;
                ...
                float4* dataColPtr = reinterpret_cast<float4*>(dataCol) + index;
                int32_t dataWeightPtr = samplingIndex * numLevels * numPoint;
                int32_t dataLocWPtr = dataWeightPtr << 1;
                int32_t const qidStride = numHeads * channels;
                int32_t const dataValuePtrInitOffset = bCol * spatialSize * qidStride;
                float const* dataValuePtrInit = reinterpret_cast<float const*>(dataValue) + dataValuePtrInitOffset;
                int32_t const cBase = cVec << 2;
                ...
                for (int32_t lCol = 0; lCol < numLevels; ++lCol)
                {
                    ...
                    int32_t const spatialH = dataSpatialShapes[spatialHPtr];
                    int32_t const spatialW = dataSpatialShapes[spatialHPtr + 1];
                    float const* dataValuePtr = dataValuePtrInit + levelStartId * qidStride;
                    for (int32_t pCol = 0; pCol < numPoint; ++pCol)
                    {
                        float const locW = dataSamplingLoc[dataLocWPtr];
                        float const locH = dataSamplingLoc[dataLocWPtr + 1];
                        ...
                        if (hIm > -1.0F && wIm > -1.0F && hIm < spatialH && wIm < spatialW)
                        {
                            float4 const val = ms_deform_attn_im2col_bilinear_float4_custom(
                                dataValuePtr, spatialH, spatialW, numHeads, channels, hIm, wIm, mCol, cBase);
                            col.x += val.x * weight;
                            col.y += val.y * weight;
                            col.z += val.z * weight;
                            col.w += val.w * weight;
...
__device__ __forceinline__ float2 load_half2_as_float2_custom(__half const* ptr)
{
    __half2 const value = *reinterpret_cast<__half2 const*>(ptr);
    return __half22float2(value);
}

__device__ __forceinline__ float2 ms_deform_attn_im2col_bilinear_half2_custom(__half const* bottomData,
    int32_t const height, int32_t const width, int32_t const nHeads, int32_t const channels, float const h,
    float const w, int32_t const m, int32_t const cBase)
{
    int32_t const hLow = floorf(h);
    ...
    if (hLow >= 0 && wLow >= 0)
    {
        v1 = load_half2_as_float2_custom(bottomData + hLowPtrOffset + wLowPtrOffset + basePtr);
    }
    float2 v2 = make_float2(0.0F, 0.0F);
    if (hLow >= 0 && wHigh <= width - 1)
    {
        v2 = load_half2_as_float2_custom(bottomData + hLowPtrOffset + wHighPtrOffset + basePtr);
    }
    float2 v3 = make_float2(0.0F, 0.0F);
    if (hHigh <= height - 1 && wLow >= 0)
    {
        v3 = load_half2_as_float2_custom(bottomData + hHighPtrOffset + wLowPtrOffset + basePtr);
    }
    float2 v4 = make_float2(0.0F, 0.0F);
    if (hHigh <= height - 1 && wHigh <= width - 1)
    {
        v4 = load_half2_as_float2_custom(bottomData + hHighPtrOffset + wHighPtrOffset + basePtr);
    }
...
__global__ void ms_deformable_im2col_gpu_kernel_half2_custom(int32_t const n, __half const* __restrict__ dataValue,
    int32_t const* __restrict__ dataSpatialShapes, int32_t const* __restrict__ dataLevelStartIndex,
    __half const* __restrict__ dataSamplingLoc, __half const* __restrict__ dataAttnWeight, int32_t const batchSize,
    int32_t const spatialSize, int32_t const numHeads, int32_t const channels, int32_t const numLevels,
    int32_t const numQuery, int32_t const numPoint, __half* __restrict__ dataCol)
{
    int32_t const vecChannels = channels >> 1;
    CUDA_KERNEL_LOOP(index, n)
    {
        int32_t _temp = index;
        int32_t const cPair = _temp % vecChannels;
        _temp /= vecChannels;
        int32_t const samplingIndex = _temp;
        int32_t const mCol = _temp % numHeads;
        _temp /= numHeads;
        ...
        int32_t const qidStride = numHeads * channels;
        int32_t const dataValuePtrInitOffset = bCol * spatialSize * qidStride;
        int32_t const cBase = cPair << 1;
        float2 col = make_float2(0.0F, 0.0F);
        ...
        for (int32_t lCol = 0; lCol < numLevels; ++lCol)
        {
            ...
            for (int32_t pCol = 0; pCol < numPoint; ++pCol)
            {
                float const locW = __half2float(dataSamplingLoc[dataLocWPtr]);
                float const locH = __half2float(dataSamplingLoc[dataLocWPtr + 1]);
                float const weight = __half2float(dataAttnWeight[dataWeightPtr]);
                float const hIm = locH * spatialH - 0.5F;
                float const wIm = locW * spatialW - 0.5F;

                if (hIm > -1.0F && wIm > -1.0F && hIm < spatialH && wIm < spatialW)
                {
                    float2 const value = ms_deform_attn_im2col_bilinear_half2_custom(
                        dataValuePtr, spatialH, spatialW, numHeads, channels, hIm, wIm, mCol, cBase);
                    col.x += value.x * weight;
                    col.y += value.y * weight;
                }
        ...
        reinterpret_cast<__half2*>(dataCol)[index] = __floats2half2_rn(col.x, col.y);
    }
}
#endif
    ...
    int32_t const numQuery, int32_t const numPoint, scalar_t* dataCol)
{
    ...
    bool usedOptimizedKernel = false;
    bool const enableOptimizedKernel = pose_msda_get_enable_optimized_kernel() != 0;
    if constexpr (std::is_same<scalar_t, float>::value)
    {
        if (enableOptimizedKernel)
        {
            if (channels == 32 && numLevels == 4 && numPoint == 4)
            {
                int32_t const numBlocks = batchSize * numQuery * numHeads;
                ms_deformable_im2col_gpu_kernel_v2_split_reduce_float32_custom<<<numBlocks, 128, 0, stream>>>(
                    dataValue, dataSpatialShapes, dataLevelStartIndex, dataSamplingLoc, dataAttnWeight, spatialSize,
                    numHeads, numQuery, dataCol);
            }
            else
            {
                int32_t numV2Kernels = numKernels;
                if ((channels & 3) == 0)
                {
                    numV2Kernels /= 4;
                }
                ms_deformable_im2col_gpu_kernel_v2_custom<scalar_t>
                    <<<GET_BLOCKS(numV2Kernels, numThreads), numThreads, 0, stream>>>(numV2Kernels, dataValue,
            ...
            usedOptimizedKernel = true;
        }
    }
    if constexpr (std::is_same<scalar_t, __half>::value)
    {
        if (enableOptimizedKernel && ((channels & 1) == 0))
        {
            int32_t const numHalf2Kernels = numKernels >> 1;
            ms_deformable_im2col_gpu_kernel_half2_custom<<<GET_BLOCKS(numHalf2Kernels, numThreads), numThreads, 0, stream>>>(
                numHalf2Kernels, dataValue, dataSpatialShapes, dataLevelStartIndex, dataSamplingLoc, dataAttnWeight,
                batchSize, spatialSize, numHeads, channels, numLevels, numQuery, numPoint, dataCol);
            usedOptimizedKernel = true;
        }
    }
    if (!usedOptimizedKernel)
    {
