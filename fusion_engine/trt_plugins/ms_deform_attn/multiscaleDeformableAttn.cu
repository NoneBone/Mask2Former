...
namespace
{
std::atomic<int32_t> gEnableOptimizedKernel{1};
}

extern "C" void pose_msda_set_enable_optimized_kernel(int32_t enable)
{
    gEnableOptimizedKernel.store(enable != 0 ? 1 : 0, std::memory_order_relaxed);
}

extern "C" int32_t pose_msda_get_enable_optimized_kernel()
{
    return gEnableOptimizedKernel.load(std::memory_order_relaxed);
}
